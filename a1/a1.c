...
void splitInput(char *input, char **args, int *arg_count) {
  *arg_count = 0;
  ...
  for (char *ptr = input; *ptr != '\0'; ptr++) {
    if (*ptr == '"') {
      in_quotes =
          !in_quotes; // Toggle the in_quotes flag when a quote is encountered
    }
  ...
  if (strcmp(table, "teams") == 0) {
    ...
    for (int i = 0; i < teams_metadata.count; i++) {
      if (!teams[i].isDeleted) {
        printf("%d,%s,%d,%s,%s,%lf\n", teams[i].id, teams[i].name,
               teams[i].yearFounded, teams[i].city, teams[i].sport,
               teams[i].popularityIndex);
      }
    }
  } else if (strcmp(table, "athletes") == 0) {
    ...
    for (int i = 0; i < athletes_metadata.count; i++) {
      if (!athletes[i].isDeleted) {
        printf("%d,%s,%d,%s,%s\n", athletes[i].id, athletes[i].name,
               athletes[i].birthYear, athletes[i].nationality,
               athletes[i].sport);
      }
    }
  } else if (strcmp(table, "competitions") == 0) {
    ...
    for (int i = 0; i < competitions_metadata.count; i++) {
      if (!competitions[i].isDeleted) {
        printf("%d,%s,%d,%s,%s\n", competitions[i].id, competitions[i].name,
               competitions[i].year, competitions[i].sport,
