...
void MusicRecordHashTable_resize(recordht_t *const ht) {
  ...
  // Create new table
  recordkvp_t **new_table =
      (recordkvp_t **)malloc(new_capacity * sizeof(recordkvp_t *));
...
void MusicRecordHashTable_insert(recordht_t *ht, record_t *const record) {
  ...
  while (ht->table[i]->in_use) {
    if ((strcmp(ht->table[i]->k_record->name, record->name) == 0) &&
        (ht->table[i]->k_record->releaseYear == record->releaseYear)) {
...
int main(int argc, char *argv[]) {
  ...
  if (strcmp(argv[1], "transformArray") == 0) {
    ...
    if ((rows * cols) + 4 != argc) {
      fprintf(
          stderr,
          "Usage: ./hw1 transformArray {rows} {cols} {rows*cols elements}\n");
      return EXIT_FAILURE;
    }
    ...
    if (num_elements + 4 != argc) {
      fprintf(
          stderr,
          "Usage: ./hw1 replaceLastNRandom {num_to_replace} {num_elements} {elements}\n");
      return EXIT_FAILURE;
    }
