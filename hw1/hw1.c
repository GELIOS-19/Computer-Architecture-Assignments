...
{
    for (int i = 0; i < ht->capacity; i++) { free(ht->table[i]); }
    free(ht->table);
}
...
void MusicRecordHashTable_insert(recordht_t* ht, record_t* const record)
{
    ...
    if (ht->used >= ht->capacity / 2) { MusicRecordHashTable_resize(ht); }
...
int numDuplicates(record_t* records, int numRecords)
{
    ...
    for (int i = 0; i < numRecords; i++) { MusicRecordHashTable_insert(ht, &records[i]); }

    int num_duplicates = 0;
    for (int i = 0; i < ht->capacity; i++)
    {
        if (ht->table[i]->in_use) { num_duplicates += ht->table[i]->v_num_duplicates; }
    }
...
void replaceLastNRandom(int* arr, int numElements, int numToReplace)
{
    ...
    {
        if (i >= numElements - numToReplace) { arr[i] = rand(); }
    }
}
...
int main(int argc, char* argv[])
{
    ...
    else if (strcmp(argv[1], "numDuplicates") == 0)
    {
        ...
        for (int i = 0; i < numRecords; i++) { free(records[i].name); }
        free(records);
    }
    else if (strcmp(argv[1], "replaceLastNRandom") == 0)
    {
        ...
        for (int i = 0; i < num_elements; i++) { arr[i] = atoi(argv[4 + i]); }
        ...
        // output should be:
        // 1 2 3 4 5
        for (int i = 0; i < num_elements; i++) { printf("%d ", arr[i]); }
        printf("\n");
