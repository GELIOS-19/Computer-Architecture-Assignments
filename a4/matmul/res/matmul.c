...
int* access_matrix(matrix_t* mat, int row, int col) { return (mat->data + (row * mat->cols) + col); }
...
void print_matrix(FILE* fp, matrix_t* mat)
{
    ...
    {
        for (int j = 0; j < mat->cols; j++) { fprintf(fp, "%d ", mat->data[(i * mat->cols) + j]); }
        fprintf(fp, "\n");
    }
}
...
matrix_t* matmul(matrix_t* left, matrix_t* right)
{
    ...
    for (int i = 0; i < left->rows; i++)
    {
        ...
        {
            for (int k = 0; k < right->cols; k++) { *access_matrix(out, i, k) += (*access_matrix(left, i, j)) * (*access_matrix(right, j, k)); }
        }
    }
    MARKER_END = 98;
...
int main(int argc, char** argv)
{
    ...
    // Input: a b c l1 l2 l3 l4... ln r1 r2 r3 ... rn (Left matrix is a x b, right matrix is b x c), rest are left and right input values respectively.
    ...
    for (int i = 0; i < a; i++)
    {
        for (int j = 0; j < b; j++)
        {
            printf("writing %d to %p\n", atoi(argv[4 + (i * b) + j]), access_matrix(left, i, j));
    ...
    for (int i = 0; i < b; i++)
    {
        for (int j = 0; j < c; j++) { *access_matrix(right, i, j) = atoi(argv[4 + (a * b) + (i * c) + j]); }
    }
    ...
    fprintf(marker_f, "%llx\n%llx", (unsigned long long int)&MARKER_START, (unsigned long long int)&MARKER_END);
