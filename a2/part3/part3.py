...
# Make sure the autograder is in the same directory as part3.c
FILE_NAME = "part3"
...
def run_test(test_number, test_input, expected_output):
    ...
    print(
        f"Test Case {test_number}: Input = {test_input}, stderr: {stderr.strip()}, Output: {expected_output.strip()}",
        end=" ",
    )
    ...
    if (
        ...
        print(
            f"   Actual Output: {stdout.strip() if stdout != '' else stderr.strip()}"
        )
        return False
...
def main():
    ...
    test_cases = [
        ...
        (5, "0000001111111111", "0.00006098"),  # changed to 6098
