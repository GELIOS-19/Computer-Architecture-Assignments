...
int main(int argc, char* argv[])
{
    ...
    /* When run with one argument <file>, the bomb reads from <file> until EOF, and then switches to standard input.
     * Thus, as you defuse each phase, you can add its defusing string to <file> and avoid having to retype it. */
    else if (argc == 2)
    {
     ...
     * No one will ever figure out how to defuse this... */
    input = read_line();
    phase_2(input);
    phase_defused();
    printf("That's number 2.  Keep going!\n");

    /* I guess this is too easy so far.
     * Some more complex code will confuse people. */
    input = read_line();
    phase_3(input);
    phase_defused();
    ...
    phase_4(input);
    phase_defused();
    printf("So you got that one.  Almost there, but not quite!\n");

    /* Did you think you were done? */
    input = read_line();
    phase_5(input);
    phase_defused();
    printf("Good work!  You defused the bomb! Congratulations!\nYour instructor has been notified and will verify your solution.\n");
